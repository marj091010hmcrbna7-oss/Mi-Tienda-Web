...
def update_store(request):
    if request.method == "GET":
        ...
        # Actualizar el producto
        name_original = request.POST.get("name_original")
        store = stores.objects.get(name=name_original)
...
def delete_store(request):
    ...
    if request.method == 'POST':
        ...
        if kword != "":
            try:
                obj = stores.objects.get(name=kword)
                obj.delete()
                context['mensaje'] = f"tienda '{kword}' eliminado correctamente."
            except stores.DoesNotExist:
                context['error'] = f"No se encontro la tienda '{kword}'."
